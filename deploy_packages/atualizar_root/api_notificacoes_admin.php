...
if ($action === 'create') {
    ...
    // Sanitizar e validar ID
    $newId = preg_replace('/[^a-zA-Z0-9_-]/', '', (string)$input['id']);
    if (empty($newId) || strlen($newId) > 80) {
        http_response_code(400);
        echo json_encode(['error' => 'Invalid id: use only letters, digits, _ and -, max 80 chars']);
        exit;
    }

    // Impedir ID duplicado
    foreach ($data as $existing) {
        if (isset($existing['id']) && $existing['id'] === $newId) {
            http_response_code(409);
            echo json_encode(['error' => 'Duplicate id: ' . $newId]);
            exit;
        }
    }

    // Limites de tamanho
    $title   = substr(trim((string)$input['title']),   0, 160);
    $body    = substr(trim((string)$input['body']),    0, 4000);
    $details = substr(trim((string)($input['details'] ?? '')), 0, 8000);
    $version = substr(trim((string)($input['version'] ?? '')), 0, 40);

    if (empty($title)) {
        http_response_code(400);
        echo json_encode(['error' => 'title cannot be empty after trimming']);
        exit;
    }
    if (empty($body)) {
        http_response_code(400);
        echo json_encode(['error' => 'body cannot be empty after trimming']);
        exit;
    }

    // Validar published como data parseable
    $publishedRaw = trim((string)$input['published']);
    $publishedTs  = strtotime($publishedRaw);
    if ($publishedTs === false || $publishedTs <= 0) {
        http_response_code(400);
        echo json_encode(['error' => 'published must be a valid ISO 8601 date']);
        exit;
    }

    // Validar expires como data parseable (se fornecido) e garantir expires > published
    $expiresRaw = isset($input['expires']) && $input['expires'] !== null && $input['expires'] !== ''
        ? trim((string)$input['expires']) : null;
    $expiresTs = null;
    if ($expiresRaw !== null) {
        $expiresTs = strtotime($expiresRaw);
        if ($expiresTs === false || $expiresTs <= 0) {
            http_response_code(400);
            echo json_encode(['error' => 'expires must be a valid ISO 8601 date']);
            exit;
        }
        if ($expiresTs <= $publishedTs) {
            http_response_code(400);
            echo json_encode(['error' => 'expires must be strictly after published']);
            exit;
        }
    }
    ...
    // Sanitizar target_license: apenas caracteres seguros
    $targetLicenseRaw = $input['target_license'] ?? null;
    $targetLicense = null;
    if (!empty($targetLicenseRaw)) {
        $targetLicense = preg_replace('/[^a-zA-Z0-9_-]/', '', (string)$targetLicenseRaw);
        if (empty($targetLicense)) $targetLicense = null;
    }
