      ...
      <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-200">
        ...
        {cashierHistory.length > 0 && (
          <div className="space-y-6">
            <div className="flex items-center gap-3"><History className="w-5 h-5 opacity-40" /><h3 className="text-sm font-bold uppercase tracking-wide opacity-40">Histórico de Fechamento</h3></div>
            <div className={`rounded-lg border overflow-hidden shadow-sm ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100'}`}>
                  ...
                  <tbody className={`divide-y ${isDark ? 'divide-[#2C2C2E]' : 'divide-gray-100'}`}>
                    {[...cashierHistory].reverse().map((s, idx) => {
                      ...
                      <tr key={idx} className={`transition-colors ${isDark ? 'hover:bg-[#252527]' : 'hover:bg-gray-50/50'}`}>
                        ...
                        <td className="px-8 py-6 text-right font-bold text-emerald-500">R$ {(s.salesTotal + s.serviceTaxTotal).toFixed(2)}</td>
                        <td className="px-8 py-6 text-right font-bold text-red-500">R$ {s.expensesTotal.toFixed(2)}</td>
                        <td className={`px-8 py-6 text-right font-bold text-xs ${diffColor}`}>
                          {hasDiff ? `${diffSign}R$ ${s.cashBreakdown?.toFixed(2)}` : 'N/C'}
                        </td>
                        <td className="px-8 py-6 text-right"><span className="px-4 py-2 bg-emerald-500/10 text-emerald-500 rounded-full font-bold text-xs">R$ {(s.finalBalance ?? 0).toFixed(2)}</span></td>
                      </tr>
                    )})}
                  </tbody>
                </table>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-200">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <CashierStatCard label="Saídas (Despesas)" value={expensesTotal > 0 ? expensesTotal : 0} icon={TrendingDown} color="rose" isDark={isDark} />
        <CashierStatCard label="Saldo Estimado Caixa" value={saldoPrevisto} icon={Wallet} color="blue" isDark={isDark} isTotal />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className={`lg:col-span-8 p-10 rounded-xl border shadow-sm flex flex-col ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100'}`}>
          ...
          <div className="space-y-6 flex-1">
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
               <div className={`p-6 rounded-lg border ${isDark ? 'bg-[#121214] border-[#2C2C2E]' : 'bg-gray-50/50 border-gray-100'}`}><div className="flex items-center gap-3 mb-4 opacity-40"><Wallet className="w-4 h-4" /><span className="text-[10px] font-bold uppercase tracking-wide">Fundo Inicial</span></div><p className="text-2xl font-bold tracking-tight text-[#475569]">R$ {initialBalance.toFixed(2)}</p></div>
               <div className={`p-6 rounded-lg border ${isDark ? 'bg-[#121214] border-[#2C2C2E]' : 'bg-gray-50/50 border-gray-100'}`}><div className="flex items-center gap-3 mb-4 opacity-40"><CreditCard className="w-4 h-4" /><span className="text-[10px] font-bold uppercase tracking-wide">Vendas de Balcão</span></div><p className="text-2xl font-bold tracking-tight text-emerald-500">R$ {salesToday.toFixed(2)}</p></div>
               <div className={`p-6 rounded-lg border ${isDark ? 'bg-[#121214] border-[#2C2C2E]' : 'bg-gray-50/50 border-gray-100'}`}><div className="flex items-center gap-3 mb-4 opacity-40"><Receipt className="w-4 h-4" /><span className="text-[10px] font-bold uppercase tracking-wide">Serviço & Taxas</span></div><p className="text-2xl font-bold tracking-tight text-emerald-500">R$ {serviceChargeToday.toFixed(2)}</p></div>
             </div>
                   ...
                   {expenses.map(e => {
                     ...
                     return (
                       <div key={e.id} className={`flex items-center justify-between p-5 rounded-lg ${isDark ? 'bg-[#121214]' : 'bg-gray-50'}`}>
                         ...
                         <span className={`font-bold ${textClass}`}>{isEntrada ? '+' : '-'} R$ {e.amount.toFixed(2)}</span>
                       </div>
                     );
                   })}
                 </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className={`p-8 rounded-xl border shadow-sm relative overflow-hidden transition-all ${!canCloseCashier ? 'border-amber-500/30 bg-amber-500/5' : 'border-red-500/20 bg-red-500/5'}`}>
            <h3 className={`text-sm font-bold uppercase tracking-wide mb-6 ${!canCloseCashier ? 'text-amber-500' : 'text-red-500'}`}>Encerramento</h3>
            ...
            {!canCloseCashier ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[9px] font-bold opacity-30 ml-2 mt-1 uppercase tracking-wide">Compara com o saldo estimado ({saldoPrevisto.toFixed(2)})</p>
                </div>
...
const CashierStatCard = ({ label, value, icon: Icon, color, isDark, isTotal }: any) => {
  const colors = {
    ...
    blue: 'text-[#475569] bg-[#475569]/10 border-[#475569]/20',
  }[color as 'emerald' | 'rose' | 'blue'];

  return (
    <div className={`p-8 rounded-lg border transition-all ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100 shadow-sm'}`}>
      <div className="flex justify-between items-start mb-6">
        <div className={`p-3 rounded-lg ${colors}`}><Icon className="w-5 h-5" /></div>
        <div className="text-right">
          <p className="text-[10px] font-bold uppercase tracking-wide opacity-40 mb-1">{label}</p>
          <div className="flex items-baseline justify-end gap-1"><span className={`text-sm font-bold opacity-30 ${isTotal ? 'text-[#475569] opacity-50' : ''}`}>R$</span><p className={`text-3xl font-bold tracking-tighter ${isTotal ? 'text-[#475569]' : ''}`}>{value.toFixed(2)}</p></div>
        </div>
      </div>
      <div className="flex items-center gap-2"><div className={`w-full h-1.5 rounded-full overflow-hidden ${isDark ? 'bg-white/5' : 'bg-gray-100'}`}><motion.div initial={{ width: 0 }} animate={{ width: '70%' }} className={`h-full rounded-full ${color === 'rose' ? 'bg-red-500' : color === 'emerald' ? 'bg-emerald-500' : 'bg-[#475569]'}`} /></div></div>
    </div>
  );
};
