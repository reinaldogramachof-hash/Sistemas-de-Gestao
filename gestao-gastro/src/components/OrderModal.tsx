                 ...
                 {isReservada ? (
                    ...
                    <div className={`w-full max-w-md rounded-xl border flex flex-col overflow-hidden shadow-sm ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100'}`}>
                      <div className={`p-8 flex justify-between items-center border-b ${isDark ? 'bg-[#252527] border-[#2C2C2E]' : 'bg-gray-50 border-gray-200'}`}>
                         <div><h3 className="font-bold text-2xl">Mesa {tableNumber?.toString().padStart(2, '0')}</h3><p className="text-xs font-semibold opacity-40">Configuracao de Abertura</p></div>
                      ...
                      <div className="p-10 space-y-8">
                        ...
                        <button onClick={handleOpenTable} className="w-full mt-6 py-5 bg-[#475569] rounded-lg text-white font-bold text-sm shadow-sm transition-all">Abrir Mesa</button>
                      </div>
                    </div>
                  ) : isManaging ? (
                    ...
                    <div className={`w-full max-w-sm rounded-xl border p-12 flex flex-col items-center text-center space-y-6 ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100 shadow-sm'}`}>
                      ...
                      <div className="space-y-2"><h3 className="text-xl font-bold">Erro de Sincronia</h3><p className="text-sm font-medium opacity-40 leading-relaxed px-4">Esta mesa est sem comanda ativa no sistema.</p></div>
          ...
          if (activeOrder) {
            return (
              <div className="flex w-full gap-6 h-full items-stretch">
                ...
                <div className={`w-[450px] rounded-xl border flex flex-col overflow-hidden shadow-sm relative z-10 ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100'}`}>
                  {/* Order Header */}
                  <div className={`p-8 border-b flex justify-between items-start ${isDark ? 'bg-[#252527] border-[#2C2C2E]' : 'bg-gray-50 border-gray-100'}`}>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => setIsManaging(true)} className={`p-3 rounded-lg border transition-all ${isDark ? 'bg-white/5 border-white/10 text-white' : 'bg-white border-gray-200 shadow-sm'}`}><Settings className="w-5 h-5 opacity-60" /></button>
                      <button onClick={onClose} className={`p-3 rounded-lg border transition-all ${isDark ? 'bg-white/5 border-white/10 text-white' : 'bg-white border-gray-200 shadow-sm'}`}><X className="w-5 h-5 opacity-60" /></button>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-8 space-y-4 custom-scrollbar">
                    <AnimatePresence initial={false}>
                      {activeOrder.items.length === 0 ? (
                        ...
                        <motion.div layout initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} key={item.id} className={`flex items-center gap-4 p-5 rounded-lg group transition-all ${isDark ? 'bg-[#121214] hover:bg-[#121214]/80' : 'bg-gray-50/50 hover:bg-gray-50 border border-gray-100/50'}`}>
                          ...
                          <div className="flex-1 min-w-0"><p className="text-sm font-bold uppercase truncate group-hover:text-[#475569] transition-colors">{item.product.name}</p><p className="text-[10px] font-bold opacity-40">R$ {item.price.toFixed(2)}</p></div>
                          <div className="flex items-center gap-3">
                            ...
                            <span className="w-5 text-center font-bold text-sm tracking-tighter">{item.quantity}</span>
                            <button onClick={() => addItemToOrder(item.product)} className="w-9 h-9 rounded-xl flex items-center justify-center border border-current/10 hover:border-[#475569] hover:text-[#475569] transition-all"><Plus className="w-4 h-4" /></button>
                          </div>
                        </motion.div>
                      ))}
                    </AnimatePresence>
                  ...
                  <div className={`p-10 border-t space-y-8 ${isDark ? 'bg-[#252527] border-[#2C2C2E]' : 'bg-gray-50 border-gray-100'}`}>
                    <div className="flex justify-between items-end">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-baseline gap-1.5"><span className="text-2xl font-bold text-[#475569] opacity-50">R$</span><span className="text-5xl font-bold text-[#475569] tracking-tighter">{activeOrder.total.toFixed(2)}</span></div>
                      </div>
                      <button disabled={activeOrder.items.length === 0} onClick={() => setCheckoutOpen(true)} className="px-10 py-6 bg-[#475569] text-white rounded-lg font-bold uppercase tracking-wide text-[12px] shadow-sm disabled:opacity-30 disabled:scale-100 disabled:shadow-none transition-all">Pagar Conta</button>
                    </div>
                  </div>
                </div>
              </div>
